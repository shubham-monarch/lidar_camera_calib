...
rosbag2pcl_dir = "rosbag2pcl"
rosbag2ply_dir = "rosbag2ply"

# for path in [rosbag2pcl_dir]:
# 		try:
# 			shutil.rmtree(path)
...
for path in [rosbag2pcl_dir, rosbag2ply_dir]:
...
with rosbag.Bag(bag_file, "r") as bag:
    ...
    for topic, msg, t in tqdm(bag.read_messages(), total=total_messages, unit='msgs'):
        ...
        #np.save(f"{rosbag2pcl_dir}/pcl_{idx}.npy", np_arr)
...
for idx, (topic, msg, t) in enumerate(reversed(bag_msgs)):
    #bag_msgs_reverved.append((topic, msg, t))
    if idx > 10:
        break
    points = pointcloud2_to_array(msg)
    save_ply(points, f"{rosbag2ply_dir}/pcl_{idx}.ply")
